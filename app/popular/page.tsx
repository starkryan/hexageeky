...
// Add popularity scores to tools
const popularTools = allTools.map(tool => ({
  ...tool,
  popularity: Math.random() * 1000 // Replace with actual popularity metric
...
export default function PopularPage() {
  ...
  const handleOpenTool = (tool: Tool & { popularity: number }) => {
    addToRecentlyViewed(tool.id)
    window.open(tool.url, '_blank')
  }

  const handleShare = async (tool: Tool & { popularity: number }) => {
    try {
                            ...
                            <div className="flex justify-between space-x-4">
                              <div className="space-y-1">
                                ...
                                <p className="text-sm font-medium mt-2">
                                  Popularity Score: {Math.floor(tool.popularity)}
                                </p>
                              </div>
                            </div>
                          </HoverCardContent>
